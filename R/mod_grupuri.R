...
mod_grupuri_ui <- function(id){
  ...
  fluidPage( 
    ...
    fluidRow(
     column( width = 4, fileInput(  inputId = ns("grupuri_upload"),
          label = "Upload grupuri", width = "300px", accept = c(".xlsx", ".xls"),
          buttonLabel = "Excel only", placeholder = "No file uploaded")    ),
    column(width = 4, uiOutput("show_toggle_missing")),
      
    column(width = 4, uiOutput(ns("show_grup_date")))
    ),
    ...
    column(width =12,uiOutput(ns("show_constituite"))),
    
   # column(width = 12, uiOutput(ns("show_neconstituite"))),
    
    column(width = 12, uiOutput(ns("show_more_data"))),
...
mod_grupuri_server <- function(id, vals){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(input$grupuri_upload, {
      ...
      output$show_grup_date <- renderUI({req(grupuri_reactive$all_names)
        if (grupuri_reactive$all_names==FALSE) {
          h5(paste("Lipseste coloana:",grupuri_reactive$missing_names,collapse = " ; "))}
                        ...
                        value = vals$report_date,autoclose = TRUE, width = "350px") }
      ...
      observe({req(grupuri_reactive$all_names==TRUE)
        grupuri_reactive$fisier_prelucrat <-  dplyr::mutate(grupuri_reactive$file_read_prel,
            data_constituire_grup = lubridate::make_date(
              year = paste0("20",
                stringr::str_sub(
                  grupuri_reactive$file_read_prel$NrGrup,
                  start = -2,
                  end = -1
                )
              ) %>% as.numeric(),
              month = stringr::str_sub(
                grupuri_reactive$file_read_prel$NrGrup,
                start = -4,
                end = -3
              ) %>% as.numeric(),
      ...
      ifelse(is.na(tipologie_conventie) & Nrcontract %in% finantari_grupuri$Nrcontract,
        ...
        output$show_toggle_missing <- renderUI({ req( grupuri_reactive$need_more_data == TRUE)
          shinyWidgets::prettyToggle(inputId = session$ns("show_missing"), width = "250px",
                                     shape = "round", status_off = "danger", status_on = "success",
                                     icon_on = icon("exclamation-triangle"), icon_off = icon("exclamation"),
                                     value = FALSE, label_on = "Hide elements below",
                                     label_off = "STOP, am nevoie de informatii actualizate. Click aici!")
        })
        
        output$show_constituite <- renderUI({ 
          fluidRow(
            column(width = 3, downloadLink(outputId = session$ns("down_grupuri_selectate"),
                                    label = "Download grupuri constituite detaliat")),
            column(width = 3, actionLink(inputId = session$ns("save_grupuri"),icon = icon("save"),
                      label = paste0("Save grupuri la data de_", input$data_grupuri))),
            
            column(width = 3, shinyWidgets::prettyToggle(inputId = session$ns("show_grupuri_constituite"), 
            ...
            column(width = 3, shinyWidgets::prettyToggle(inputId = session$ns("show_grupuri_NEconstituite"), 
                    width = "250px", label_on = "Hide table below",
                    label_off = "Show grupurile NEconstituite",
                    shape="curve",status_on = "success", status_off = "info",
                    icon_on = icon("wye-slash"), icon_off = icon("table"), value = FALSE) ),
          )
        })
      ...
      output$grupuri_selectate <- DT::renderDataTable({ req( grupuri_reactive$need_more_data == FALSE)
            DT::datatable(data = grupuri_reactive$grupuri_selectate %>% 
                  ...
                  rownames = FALSE, options =  list(dom = "Bt"),extensions = 'Buttons',
                          caption = htmltools::tags$caption(style = 'caption-side: top; text-align: left;',
                            paste0('Grupuri constituite la data de ', input$data_grupuri))) %>%
        ...
        output$grupuri_detaliate <- DT::renderDataTable({ 
          ...
          DT::datatable(
            ...
            caption = paste0( "Lista grupurilor valabile la data de ",
              as.character(input$data_grupuri)),
            filter = list(position = "top", clear = TRUE, plain = TRUE),
            options = list(dom = "t", autoWidth = TRUE,   scrollY = TRUE,
              scrollX = TRUE,   pageLength = 5,   searchHighlight = TRUE,
              fixedColumns = list(leftColumns = 4),
              columnDefs = list(list(width = '100px', targets = 4),
                list(width = '175px', targets = 3) ) )) }
          else if (input$show_grupuri_NEconstituite == TRUE) {
            DT::datatable(
              data = grupuri_reactive$grupuri_neconstituite %>%  dplyr::mutate(tipologie_conventie = 
                          ifelse(is.na(tipologie_conventie),"neidentificata",tipologie_conventie)) %>%
                dplyr::mutate_at(.vars = "tipologie_conventie", as.factor) %>%
                dplyr::select(nume_obligatorii_grupuri,
                  data_constituire_grup,  tipologie_conventie  ),
              rownames = FALSE,    caption = paste0(
                "Lista grupurilor NEconstituite la data de ",input$data_grupuri ),
              filter = list(position = "top", clear = TRUE,  plain = TRUE ),
              options = list( autoWidth = TRUE,  scrollY = TRUE,  scrollX = TRUE,
                dom = "t",  pageLength = 5,   searchHighlight = TRUE,
                fixedColumns = list(leftColumns = 4),  columnDefs = list(
                  list(width = '100px', targets = 4),  list(width = '175px', targets = 3)
                ) )  ) }
        })
    ...
    observeEvent(input$show_missing,{ 
      output$show_more_data <- renderUI({ req(input$show_missing==TRUE)
        fluidRow(
          column(width = 12, textOutput(ns("mesaj_bi_grupuri")), hr()),
          
          column(width = 6, fileInput(inputId = session$ns("input_bi_grupuri"),label = "Upload BI",accept = ".xlsx",
                                      buttonLabel = "Excel only",placeholder = "No file uploaded")),
          
          column(width = 6, br(),uiOutput(session$ns("show_bi_link"))),
          
          column(width  = 12, DT::dataTableOutput(session$ns("grupuri_mising_info")))
          )     })
      
      output$mesaj_bi_grupuri <- renderText({req(input$show_missing==TRUE)
      "Se uploadeaza un BI cu un snapshot cat mai recent pentru actualizarea automata a datelor.
                                            Snapsot-ul se actualizeaza in primul rand in sheet-ul baza_contracte si optional in sheet-ul
                                            tipologie_contracte doar daca au aparut conventii noi." })
      
      output$show_bi_link <- renderUI({
        downloadLink(outputId = session$ns("link_bi_grupuri"),label = "Click aici pentru a downloada modelul de BI")
      })
      ...
      output$grupuri_mising_info <- DT::renderDataTable(
        DT::datatable(data =  grupuri_reactive$filtered_data %>% dplyr::filter(is.na(tipologie_conventie) & 
                      Nrcontract!="-"), options = list(pageLength = 6, dom = "Ftp"),filter = "top",
            caption = "Nu se constituie grupurile de mai jos", rownames = F)
      )
      ...
      if (nrow(bi_grupuri()) < nrow(grupuri_reactive$bi_grupuri) )  {
        ...
        "Baza de date a fost actualizata cu succes. Poti sa inchizi de mai sus aceste
                    elemente de dialog si sa mai uploadezi inca o data fisierul de grupuri" })
      }
      })
      ...
      observeEvent(input$save_grupuri, {
        shinyWidgets::ask_confirmation( inputId = session$ns("confirm_save"),
          title = "CONFIRM", text = paste0("Esti sigur ca vrei sa salvezigrupurile la data de ",
            input$data_grupuri), type = "info",
          btn_labels = c("NU, renunta", "OK, salveaza"),
