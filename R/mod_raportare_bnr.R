...
mod_raportare_bnr_ui <- function(id){
  ...
  bs4Dash::tabBox(width = 12,  
    ...
    fluidRow(
      ...
      column(width = 6, br(), shinyWidgets::prettyToggle( inputId = ns("show_pivot"),
        label_off = "Vezi modelul de pivot", label_on = "Ascunde modelul de pivot",icon_off = icon("table"),
        icon_on = icon("eye-slash"),value = FALSE, status_off = "primary",status_on = "warning",
        shape = "curve",outline =  TRUE,fill = TRUE,plain = TRUE,animation = "rotate")) ),
...
mod_raportare_bnr_server <- function(input, output, session){
  ...
  output$model_sold_pc <-  DT::renderDataTable( { req(input$show_pivot == TRUE)
      ...
      Columns = c("", "",""), Values = c(  "Sum of Sold Credit (Lei)",    "Sum of Sold Garantie (Lei)",
        "Count of Cod Partener" ) ),htmltools::tags$caption( style = 'caption-side: top; text-align: left;',
