...
knitr::opts_chunk$set(echo = FALSE,size = "large")
...
flextable::flextable(data = params$tabel1 %>% dplyr::mutate(dplyr::across(.cols = dplyr::starts_with("Variat"),
  ~ .x * 100)) %>% dplyr::rename_with(.data = .,.cols = dplyr::everything(),.fn = ~ stringr::str_replace_all( pattern = "_", 
    string = .x, replacement = " "))) %>% 
  flextable::colformat_double(j = 5:8, suffix = "%") %>% flextable::colformat_double(j = 3,digits = 0) %>%
  flextable::theme_vanilla() %>% flextable::width(j = 1,width = 1.2,unit = "in") %>% 
  flextable::width(j = 2,width = 0.8,unit = "in") %>%
   flextable::width(j = 3,width = 1.13,unit = "in") %>%
  flextable::width(j = 4:8,width = 0.75,unit = "in") %>% flextable::width(j = 3,width = 1,unit = "in") %>% flextable::fontsize(j = 4:8,size = 9,part = "header")
...
flextable::flextable(data = params$tabel3 %>% dplyr::mutate(dplyr::across(.cols = dplyr::starts_with("Utilizare"),
    ~ .x * 100)) %>% setNames(nm = stringr::str_replace_all(string = colnames(.),pattern = "_",replacement = " ")) ) %>% flextable::colformat_double(j = 3:5, suffix = "%") %>%
  flextable::theme_vanilla() %>% flextable::width(j = 2:5,width = 1.25,unit = "in") %>% flextable::width(j = 1,width = 1,unit = "in")
...
*Tabelul 4 – Sinteză solduri de garanţii şi surse de finanţare – August 2021*   
...
flextable::flextable(data = params$tabel7) %>%  flextable::colformat_double(j = 8, suffix = "%",digits = 2) %>% 
  ...
  flextable::width(j = 1:4,width = 1.9,unit = "in") %>% flextable::width(j = 5,width = 3,unit = "in") 
#%>%   flextable::fontsize(j = 5,size = 8,part = "footer")
```
...
tryCatch(expr = {
flextable::flextable(data = params$tabel10 %>% dplyr::mutate(dplyr::across(.cols = dplyr::starts_with("Pondere"),
      .fns = ~.x*100)) %>% setNames(nm = stringr::str_replace_all(string = colnames(.),pattern = "_",replacement = " ")) ) %>%  
    flextable::colformat_double(j = 3:5, suffix = "%",digits = 1) %>%
     flextable::theme_vanilla() %>% flextable::width(j = 1,width = 2,unit = "in") %>% flextable::width(j = 2:5,width = 1.05,unit = "in") }, error = function(e) matrix("Check Tabel 10 in plasamente",nrow = 1,ncol = 5) %>% data.frame() %>% flextable::flextable(data = .) )
...
flextable::flextable(data = params$tabel11 %>% dplyr::mutate(dplyr::across(.cols = dplyr::starts_with("Rata_anualizata"),
      .fns = ~.x*100)) %>% dplyr::mutate(dplyr::across(.cols = dplyr::starts_with("Anul"), ~as.character(.x))) ) %>% 
...
``` {r}
flextable::flextable(data = data.frame("Director Executiv Direcţia risc" = "Cristina MARINESCU",
                                       'Întocmit - Şef Serviciu Risc' = 'Marius  TIŢA',check.names = FALSE)) %>%
  flextable::align(j = 1,align = "left",part = "all") %>% flextable::align(j = 2,align = "right", part = "all") %>%
